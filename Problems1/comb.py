...
def perm(s):
	...
	for i in range(len(s)):
		t = t + [s[i] + p for p in  perm(s[:i]+ s[i+1:])]	
	return t
...
#def getComb():
#	valid_range = range(65,91) + range(98,123) + [32]
...
#			bi = str(i) + str(j)
#			key = strxor(chr(i),chr(j))
#
#
#			if key not in dictio.keys():
#				dictio[key] = ()
#
#			if bi in dictio[key] or bi[::-1] in dictio[key]
#				continue
#
#			dictio[key] = dictio[key] + (bi,)
#
#	for count in range(8):
#		new_dict = {}
#		for s in dictio.keys():
#			for l in valid_range:
#				new_key = strxor(s,chr(l))
#				
#				new_list = remove_perm([chr(l) + a for a in dictio[s]])
#
#				if [k for k in perm(new_key) if k in new_dict.keys()]:
#					continue
#				new_dict[new_key] = strxor(dictio[s],chr(l))
#		dictio = new_dict

	new_dict = {}
	for k in dictio.keys():
		if dictio[k] not in new_dict.keys():
			new_dict[dictio[k]] = ()
		new_dict[dictio[k]] = new_dict[dictio[k]] + (k,)

	return new_dict


def getAll():
	return (getC1(),getC2(),getC3(),getC4(),getC5(),getC6(),getC7(),getC8(),getC9(),getC10())
